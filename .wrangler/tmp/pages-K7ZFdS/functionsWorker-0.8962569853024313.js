...
// api/chat.js
var MISTRAL_URL = "http://oryele-mistral-alb-1544237830.us-east-1.elb.amazonaws.com/v1/chat/completions";
var MISTRAL_URL_LOCAL = "http://localhost:8000/v1/chat/completions";
var MISTRAL_MODEL = "mistral";
...
async function onRequestPost2(context) {
  ...
  try {
    ...
    if (!first || !email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return json2({ error: "Please provide your name and a valid email address." }, 400, request);
    }
    ...
    return json2({ ok: true, email_status: res.status, email_ok: res.ok }, 200, request);
  } catch (err) {
    return json2({ error: "Server error: " + (err.message || err) }, 500, request);
  }
}
...
function json2(data, status, request) {
  return new Response(JSON.stringify(data), {
    status,
    headers: { "Content-Type": "application/json", ...corsHeaders2(request) }
  });
}
__name(json2, "json");
...
async function onRequestPost3(context) {
  ...
  try {
    ...
    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return json3({ error: "Invalid email address" }, 400, request);
    }
    ...
    if (env.SUBSCRIBERS) {
      ...
      if (existing) {
        return json3({ ok: true, message: "Already subscribed" }, 200, request);
      }
    ...
    return json3({ ok: true }, 200, request);
  } catch (err) {
    return json3({ error: "Server error" }, 500, request);
  }
}
...
function json3(data, status, request) {
  return new Response(JSON.stringify(data), {
    status,
    headers: { "Content-Type": "application/json", ...corsHeaders3(request) }
  });
}
__name(json3, "json");
...
// ../.wrangler/tmp/pages-K7ZFdS/functionsRoutes-0.030613874171087407.mjs
var routes = [
  {
    routePath: "/api/chat",
    mountPath: "/api",
