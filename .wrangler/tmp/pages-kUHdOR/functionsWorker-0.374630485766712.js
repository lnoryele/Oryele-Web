...
async function onRequestPost2(context) {
  ...
  try {
    ...
    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return json2({ error: "Invalid email address" }, 400, request);
    }
    ...
    if (env.SUBSCRIBERS) {
      ...
      if (existing) {
        return json2({ ok: true, message: "Already subscribed" }, 200, request);
      }
    ...
    return json2({ error: "Server error" }, 500, request);
  }
}
...
// ../.wrangler/tmp/pages-kUHdOR/functionsRoutes-0.2517774915179848.mjs
var routes = [
  {
    routePath: "/api/chat",
    mountPath: "/api",
